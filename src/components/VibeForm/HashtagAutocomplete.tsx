  ...
  return (
    <div className="relative font-mono" ref={dropdownRef}>
      <div className="flex justify-between items-center mb-1">
        <label className="block text-amber-400 font-bold uppercase text-xs flex items-center gap-1">
          <span>[HASHTAG_HELPER]:</span>
        </label>
      ...
      >
        {selectedTags.map((tag, idx) => (
          <span
            key={idx}
            className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded font-bold transition-colors ${
              idx === 0
                ? 'bg-amber-950/80 border border-amber-500/80 text-amber-300'
                : 'bg-zinc-800 border border-zinc-700 text-cyan-300'
            }`}
          >
            <span>{tag}</span>
            {idx === 0 && <span className="text-[9px] text-amber-400 font-normal">★ 1st</span>}
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
      ...
      {isOpen && filteredSuggestions.length > 0 && (
        <div className="absolute left-0 right-0 top-full mt-1 max-h-48 overflow-y-auto bg-zinc-950 border border-zinc-800 rounded shadow-2xl z-50 p-1 divide-y divide-zinc-900/60">
