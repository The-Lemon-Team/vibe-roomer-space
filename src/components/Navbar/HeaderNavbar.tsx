...
import { useAtmosphericStore, ATMOSPHERIC_CONTEXT_MAP } from '../../store/useAtmosphericStore';

export const HeaderNavbar: React.FC = () => {
  const { currentContext, config, viewMode, setViewMode, setCreateModalOpen } = useAtmosphericStore();
  ...
  return (
    <header className="flex justify-between items-center w-full px-4 md:px-6 py-2.5 h-16 sticky top-0 z-40 bg-zinc-950/90 backdrop-blur-md shadow-[0_0_10px_rgba(0,219,233,0.1)] border-b border-zinc-800 font-mono">
      ...
      <div className="flex items-center space-x-3">
        <span className="font-sans italic font-black text-xl md:text-2xl text-cyan-400 tracking-tighter">
          VIBER ROOMER
        </span>
      ...
      {/* Middle Active Vibe Indicator */}
      <div className="hidden md:flex items-center space-x-2 text-xs">
        <span className="text-zinc-500">[ACTIVE_MODE]:</span>
        <span 
          className="font-bold px-2 py-0.5 rounded bg-zinc-900 border uppercase"
          style={{ borderColor: config.neonColor, color: config.neonColor }}
        >
          {ATMOSPHERIC_CONTEXT_MAP[currentContext].label}
        </span>
      </div>
